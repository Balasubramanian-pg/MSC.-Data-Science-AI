...
```mermaid
flowchart LR
    A[Story Structure] --> B[Emotional Engagement]
    B --> C[Attention]
    C --> D[Meaning Formation]
...
flowchart LR
    ...
    B --> C[Narrative Context]
    C --> D[Emotional Relevance]
...
```mermaid
flowchart LR
    A[More Elements] --> B[Higher Cognitive Load]
    B --> C[Reduced Understanding]

    D[Selective Simplicity] --> E[Focused Attention]
...
```mermaid
flowchart LR
    A[One Core Idea] --> B[Focused Attention]
    B --> C[Clear Understanding]
    C --> D[Better Recall]
```
...
```mermaid
flowchart LR
    A[Critical Data] --> B[Poor Presentation Design]
    B --> C[Misinterpretation]
    C --> D[Underestimated Risk]
...
```mermaid
flowchart TD
    A[Large High-Contrast Headline]
    --> B[Critical Risk Information]
...
```mermaid
flowchart LR
    A[Raw Information] --> B[Structured Narrative]
    B --> C[Visual Clarity]
    C --> D[Focused Attention]
    D --> E[Understanding]
    E --> F[Memory]
    F --> G[Decision]
    G --> H[Action]
