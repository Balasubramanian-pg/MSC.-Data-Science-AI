...
```mermaid
flowchart TD
    A[Trend Detected Over Time] --> B[Drill Into Specific Regions]
    B --> C[Identify Local Hotspots]
    C --> D[Zoom Out For Regional Context]
    D --> E[Recommend Policy Action]
```
...
```mermaid
flowchart LR
    A[Previous Year Total] --> B[Increase Factor]
    B --> C[Decrease Factor]
    C --> D[Net Result]
```
...
```mermaid
flowchart TD
    A[Trend Over Time] --> B[Detect Intersection]
    B --> C[Contrast Regions]
    C --> D[Identify Outlier]
    D --> E[Analyze Contributing Factors]
    E --> F[Zoom Out For National Context]
```
