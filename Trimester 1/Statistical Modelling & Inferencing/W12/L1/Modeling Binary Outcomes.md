...
1. **Probability Boundary Violations:** A probability _must_ be between 0 and 1. Because the LPM is a straight line, it will inevitably predict values below 0 or above 1 for extreme values of $X$. This is logically nonsensical (e.g., a "negative 20% chance" of treatment response).
    
2. **Heteroskedasticity:** In OLS, we assume the error term $\epsilon$ has constant variance. In a binary model, the variance of the error term depends on the probability itself: $\sigma^2 = p(1-p)$. As your probability moves toward 0 or 1, the variance shrinks, which violates the OLS assumption of homoskedasticity. This makes your standard errors (and thus your t-tests/p-values) unreliable.
    
3. **Non-Linearity:** In reality, the relationship between a predictor and the probability of success is rarely a straight line. For example, moving from a 90% to a 95% response rate is much harder to achieve than moving from 50% to 55%. The LPM forces a constant, linear rate of change that rarely mirrors real-world pharmaceutical dynamics.
...
|**Statistical Rigor**|Low (for binary data)|High (Standard for binary response)|
...
However, for any model intended for **forecasting, clinical trial interpretation, or risk assessment**, the LPM is generally considered inappropriate. You need a model that honors the $[0, 1]$ boundaries of probability and the non-linear nature of conversion or response.

**Would you like to move on to Section 1.2, where we introduce the Logistic Function (the S-curve) that solves these boundary and linearity problems, or are you interested in how we use MLE specifically to estimate the parameters of a Logistic Regression?**


### 1.2 The Flaws of the Linear Probability Model (LPM)

The Linear Probability Model (LPM) is a classic example of why mathematical convenience—using standard OLS regression—does not always translate into statistical validity. While the coefficients in an LPM are easy to explain, the model fundamentally misrepresents the nature of binary data.

#### 1. Nonsensical Predictions: The [0,1] Boundary Violation
...
The Probit model replaces the logistic function with the **Cumulative Distribution Function (CDF) of the Standard Normal Distribution**, denoted as $\Phi$:
...
Because these models are non-linear, we cannot use Ordinary Least Squares (OLS) to find the coefficients. OLS requires a closed-form algebraic solution that doesn't exist for the sigmoid curve.
...
### 3. Interpreting Logit Coefficients: Bridging the Intuition Gap
...
- **Example:** In a clinical study, if the coefficient for "Treatment A" is $\beta_1 = 0.693$, then $e^{0.693} \approx 2.0$.
    
- **The Insight:** "Patients taking Treatment A have **twice the odds** of recovery compared to the control group."
    

### Strategy Summary for Interpretation
...
- **The Percentage Rule:** You can quickly convert any Odds Ratio to a percentage change using the formula: $(\text{OR} - 1) \times 100$.
    
    - In your example: $(1.051 - 1) \times 100 = 5.1\%$.
