...
- **The Fundamental Tension**: We desire to know the **Population Parameter** ($\theta$), such as the true mean height of the Indian population ($\mu$).
...
- **The Core Question**: How do we quantify the gap between $\hat{\theta}$ and $\theta$, and how do we ensure our sample is a high-fidelity representation of the population?
...
- **Formula (Known Variance)**: $CI = \bar{x} \pm Z_{\alpha/2} (\frac{\sigma}{\sqrt{n}})$
...
The derivation relies on matching the **variance of a sample variance** to a Chi-squared ($\chi^2$) distribution.
