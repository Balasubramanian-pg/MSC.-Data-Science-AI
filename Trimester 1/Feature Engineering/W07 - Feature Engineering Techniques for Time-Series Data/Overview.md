...
- **Financial Indicators:** Beyond simple lags, indicators like the **Relative Strength Index (RSI)** detect momentum, and **Bollinger Bands** quantify volatility relative to a moving average. These features condense price action into "oversold" or "overbought" signals.
...
1. **Avoid Data Leakage:** This is the most common failure. **Never** include future data in your feature construction. For example, when calculating a rolling mean, ensure you are using a lagging window ($t-W$ to $t-1$) rather than a centered window (which would include $t$).
