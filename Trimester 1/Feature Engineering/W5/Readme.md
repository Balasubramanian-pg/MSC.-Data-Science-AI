...
- **TF-IDF Data Leakage:** A critical engineering error is calling `.fit_transform(X)` on the entire dataset *before* the train/test split. The IDF denominator will "see" the frequency of words in the test set. 
