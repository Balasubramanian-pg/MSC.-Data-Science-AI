...
### **Feature Engineering Master Key**

| Question | Correct Answer | Confidence Score | Key Rationale |
| --- | --- | --- | --- |
| **Q1** | The data scientist is correct only if 'Age' is used because it carries domain-relevant predictive information for the task | **100%** | Transforming a raw timestamp into a structural semantic metric like "Age" purposefully captures predictive, domain-specific behavioral signals, elevating the operation from technical formatting (preprocessing) to feature engineering. |
| **Q2** | Robust Scaling, because the outliers in Monetary Value would distort Min-Max bounds and compress normal values, while Robust Scaling uses median and IQR to minimize this distortion | **100%** | Distance-based algorithms like K-Means are highly sensitive to unscaled magnitudes. Min-Max scaling compresses standard observations into a uselessly narrow band when extreme outliers exist, whereas Robust Scaling utilizes median and Interquartile Range (IQR) to insulate the scale from distortion. |
| **Q3** | The extreme outlier (9,50,000) becomes the maximum, so the formula (X - min)/(max - min) produces very small values for all normal entries; Robust Scaling using median and IQR would be a better choice | **100%** | The mathematical structure of Min-Max Scaling utilizes the absolute maximum value as its denominator boundary. A massive outlier forces the denominator to be disproportionately large, mathematically crushing all normal variations down close to zero. |
| **Q4** | The added features introduced overfitting by increasing model complexity beyond what the data supports, leading to poor generalization | **100%** | A massive jump in training accuracy ($78\%$ to $94\%$) paired with a steep crash in test accuracy ($76\%$ to $61\%$) is the textbook signature of overfitting. High-degree polynomial interaction terms exponentially increase structural complexity, causing the model to memorize variations instead of generalizing signals. |
| **Q5** | No, because equal-width binning divides by numeric range and would create very uneven bin populations on skewed price data | **100%** | Equal-width splits the coordinate span into equal spatial intervals, which clusters skewed values into a single massive bucket while leaving trailing buckets virtually empty. Equal-frequency binning ignores distance entirely, sorting and slicing the data by index percentiles to force balanced population distributions. |
| **Q6** | Standardization ensures each feature contributes proportionally to distance calculations, preventing high-magnitude features from dominating | **100%** | K-Means relies entirely on Euclidean coordinate distances. If features retain raw, disparate metrics (such as small age spans vs. massive monetary spend numbers), the larger spatial dimension mathematically overpowers the distance equations, making the clustering outcome completely ignore the smaller feature. |
| **Q7** | The transformed feature will have mean 0 and standard deviation 1, but the right-skewed shape of the distribution will remain unchanged | **100%** | Z-score scaling is a strictly linear calculation ($Z = \frac{x - \mu}{\sigma}$). It alters numerical centering and scale variance, but it cannot warp or change the underlying geometric profile of the distribution; a skewed distribution remains structurally skewed. |
| **Q8** | Feature creation — deriving a new domain-informed feature from existing ones | **100%** | Combining two separate existing metrics structurally (such as building a mathematical ratio of Income / Loan Amount) to isolate a distinct, unified risk dimension represents feature creation. |
| **Q9** | Feature creation | **100%** | Slicing apart a raw chronological timestamp string to pull out an entirely separate, elapsed measurement metric (Days Since Last Purchase) is a textbook application of feature creation. |
| **Q10** | Apply log transformation first to compress the large value and reduce skewness, then apply Z-score standardization to center and scale the now-symmetric distribution | **100%** | Log operations require strictly positive inputs; if Z-score standardization is applied first, it shifts the distribution mean to zero, creating negative numbers that cause the subsequent log calculation to fail mathematically. Therefore, the logarithmic compression must execute first. |
